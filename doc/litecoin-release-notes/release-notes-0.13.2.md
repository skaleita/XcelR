XcelR Core version 0.13.2 is now available from:

  <https://download.xcelr.org/xcelr-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a xcelr
...
ECDSA signatures inside XcelR transactions now use validation using
...
Previous versions of XcelR Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of XcelR Core was to raise the default minimum
relay fee.

XcelR Core 0.13.2 will have a strict maximum size on the mempool. The
...
XcelR Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running xcelrd without having to do any manual configuration.
...
XcelR Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. XcelR Core will relay transactions with
...
entirely in XcelR Core 0.13, and it is at this time undecided whether the
...
XcelR Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
XcelR Core automatically creates a hidden service to listen on, without
manual configuration. XcelR Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if XcelR Core is listening, and
...
XcelRd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals XcelR
Core to use floating fees. By default, XcelR Core will use floating
fees.
...
At all times, XcelR Core will cap fees at `-maxtxfee=<x>` (default:
0.10) XCL.
Furthermore, XcelR Core will never create transactions paying less than
...
`xcelr.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /xcelrrpc>
        ...
        # optional bypass xcelrd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< xcelrrpc:<password>
    </Location>
...
Restarting xcelrd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in xcelr.conf
...
For example:
    ...
    $ src/xcelr-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The XcelR Core code base has
...
- `xcelr-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `xcelr-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the XcelR scripting system that in combination with
...
XcelR transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of XcelR Core.
...
The two phases are distinct in the XcelR-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming XcelR wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each XcelR block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of xcelrs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of xcelrs being spent is exactly
...
- **Increased security for multisig:** XcelR addresses (both P2PKH addresses
  ...
  believed to be provided by XcelR's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of XcelR's current ledger (called the UTXO set),
  ...
  note that XcelR Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  XcelR users to individually opt-in to almost any change in the XcelR
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and XcelR Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the XcelR
client to XcelR Core 0.13.2 to signal for segwit support. When a
...
Since XcelR Core 0.10.0, nodes have defaulted to only relaying and
...
- `xcelr-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `xcelr-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
