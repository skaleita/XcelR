XcelR Core version *0.15.1* is now available from:

  <https://download.xcelr.org/xcelr-0.15.1/>
...
A number of changes to the way XcelR Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 cels per kB (0.00001 XCL). This is to prepare for dropping the minimum transaction fee to 0.1 cels per kB (0.0001 XCL) in the next release.
